...
function regExpLien(arg) {
    champ = $("#" + arg.id);
    let regexLong = /^https:\/\/(www\.)?youtube\.com\/watch\?v=([\w-]+).*$/;
    let regexLongNotSecure = /^http:\/\/(www\.)?youtube\.com\/watch\?v=([\w-]+).*$/;
    let regexCourt = /^http:\/\/youtu\.be\/([\w-]+)$/;

    if (regexLong.test(champ.val())) {
        champ.css("border", "2px green solid");
        $verifLien = true;
    } else if (regexLongNotSecure.test(champ.val())) {
        champ.css("border", "2px green solid");
        $verifLien = true;
